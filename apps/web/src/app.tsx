...
export function App({ children }: { children?: ReactNode }) { const [state, dispatch] = useReducer(appReducer, initialAppState, base => ({ ...base, locale: window.localStorage.getItem('forge-agent.locale') === 'en-US' ? 'en-US' : 'zh-CN' })); const selected = demoProjects.find(item => item.id === state.projectId); const value = useMemo(() => ({ state, dispatch }), [state]); useEffect(() => { window.localStorage.setItem('forge-agent.locale', state.locale); document.documentElement.lang = state.locale }, [state.locale]); const content = children ?? (!state.mode ? <HomePage /> : state.mode === 'workspace' ? <WorkspacePage /> : state.mode === 'issue' ? <IssuePage /> : <ChatPage />); return <AppContext.Provider value={value}><div className="app-shell"><AppHeader locale={state.locale} projectName={selected?.name} onLocaleChange={locale => dispatch({ type: 'localeChanged', locale })} />{content}</div></AppContext.Provider> }
