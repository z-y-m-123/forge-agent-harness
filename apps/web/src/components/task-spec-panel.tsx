...
export function TaskSpecPanel({ onApproved, outcome }: { onApproved?: () => void; outcome?: string } = {}) {
  ...
  const targetOutcome = outcome || (github ? `先探索 ${github.repository} 的已读取上下文，再由用户确认具体改动目标。` : '为 API 服务补充请求重试策略，并保留现有错误语义。')
  ...
  return <section className="task-panel">
    ...
    <div className="spec-grid">
      <div><label>目标结果</label><p>{targetOutcome}</p><label>{github ? '候选范围' : '范围内文件'}</label><p className="file-line">{candidateFiles.map(file => <span key={file}><FileCode2 size={16} />{file}<br /></span>)}</p></div>
